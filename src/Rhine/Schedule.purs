...
hoistSchedule
  ...
  -> Schedule m1 cl1 cl2
  -> Schedule m2 cl1 cl2
hoistSchedule hoist (Schedule _ s) = Schedule initSchedule'
  where
    initSchedule' cl1 cl2 = hoist
      $ first (hoistMSF hoist) <$> s.initSchedule cl1 cl2
    hoistMSF = morphS
...
flipSchedule
  ...
  => Schedule m cl1 cl2
  -> Schedule m cl2 cl1
flipSchedule (Schedule _ s) = Schedule initSchedule_
  where
    initSchedule_ cl2 cl1 = (\x -> first (arr (second swapEither) <<< x)) <$> s.initSchedule cl1 cl2
...
rescaledSchedule schedule@(Schedule _ s) = Schedule $ initSchedule'
  where
    initSchedule' cl1 cl2 = s.initSchedule (rescaledScheduleS schedule) (rescaledClockToS cl1) (rescaledClockToS cl2)
...
-- rescaledScheduleS (Schedule _ t) = Schedule initSchedule'
--   where
--     initSchedule' (RescaledClockS cl1 rescaleS1) (RescaledClockS cl2 rescaleS2) = do
--       (runningSchedule /\ initTime ) <- t.initSchedule cl1 cl2
...
rescaledScheduleS ::
                    Monad m =>
                    Schedule m cl1 cl2 ->
                      Schedule m (RescaledClockS m cl1 time tag1)
                        (RescaledClockS m cl2 time tag2)
rescaledScheduleS (Schedule _ s)  = Schedule initSchedule'
  where initSchedule' (RescaledClockS cl1 rescaleS1)
          (RescaledClockS cl2 rescaleS2)
          = do (runningSchedule /\ initTime) <- s.initSchedule cl1 cl2
               (rescaling1      /\ initTime') <- rescaleS1 initTime
               (rescaling2      /\ _) <- rescaleS2 initTime
               let runningSchedule'
                     = runningSchedule >>>
                         (arr
                            (\ (time /\ tag12) ->
                               case tag12 of
                                   Left tag1 -> (Left) ((tag1 /\ time))
                                   Right tag2 -> (Right) ((tag2 /\ time)))
                            >>>
                            (((arr (\ (tag1 /\ time) -> (time /\ tag1))) >>>
                                (rescaling1) >>>
                                  (arr (\ (time' /\ tag1') -> (time' /\ Left tag1'))) >>> (returnA))
                               |||
                               ((arr (\ (tag2 /\ time) -> (time /\ tag2))) >>>
                                  (rescaling2) >>>
                                    (arr (\ (time' /\ tag2') -> (time' /\ Right tag2'))) >>>
                                      (returnA))))
               return (runningSchedule' /\ initTime')
...
data SequentialClock m cl1 cl2
  = SequentialClock
    { sequentialCl1      :: cl1
    , sequentialCl2      :: cl2
    , sequentialSchedule :: Schedule m (Time cl1 ~ Time cl2) cl1 cl2
    }
...
instance seqClock :: (Monad m, Clock m cl1 time1 tag1 diff, Clock m cl2 time2 tag2 diff)
      => Clock m (SequentialClock m cl1 cl2) time1 (Either (Tag cl1) (Tag cl2)) diff where
  initClock (SequentialClock s)
    = initSchedule s.sequentialSchedule s.sequentialCl1 s.sequentialCl2
...
schedSeq1 = Schedule { initSchedule = init }
  where
    init cl1 (SequentialClock sc@{ sequentialSchedule: ss })
      = do
        (runningClock /\ initTime) <- ss.initSchedule (cl1 <> sc.sequentialCl1) sc.sequentialCl2
